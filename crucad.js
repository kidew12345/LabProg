...
import { StyleSheet,TouchableOpacity, View, Alert, Platform, ActivityIndicator,ScrollView } from 'react-native';
...
constructor(props) {
 ...
 insertRecord = () =>{
      fetch('http://192.168.43.41/crud-react/InsertStudentData.php', {
      method: 'POST',
      headers: {
  ...
  if (this.state.isLoading) {
   ...
   return (
    ...
    class ShowStudentListActivity extends Component {
      ...
      componentDidMount() {
          return fetch('http://192.168.43.41/crud-react/ShowAllStudentsList.php')
            .then((response) => response.json())
            .then((responseJson) => {
              let ds = new View.DataSource({rowHasChanged: (r1, r2) => r1 !== r2});
              this.setState({
                isLoading: false,
            ...
            <View style={styles.MainContainer_For_Show_StudentList_Activity}>
              <ListView
                ...
                <TouchableOpacity style   = {{ backgroundColor : 'white', padding : 10 }}
                    ...
                    <Text style = { styles.FlatListContainer } > 
                      {rowData.student_name} 
                    </Text> 
...
class EditStudentRecordActivity extends Component {
  ...
  constructor(props) {
    ...
    updateRecord = () =>{
      this.setState({
        isLoading: true
            ...
            fetch('http://192.168.43.41/crud-react/UpdateStudentRecord.php', {
            method: 'POST',
            headers: {
              'Accept': 'application/json',
              'Content-Type': 'application/json',
    ...
    deleteRecord = () =>{
          fetch('http://192.168.43.41/crud-react/DeleteStudentRecord.php', {
          method: 'POST',
          headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
